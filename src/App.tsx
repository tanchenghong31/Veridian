    ...
    return (
      <div className="space-y-2">
        ...
        <div className="flex items-center space-x-2">
          <input 
            className="flex-1 p-2 border border-stone-200 rounded-sm text-xs bg-white"
            value={value}
            onChange={(e) => onChange(e.target.value)}
            placeholder="Image URL or upload"
          />
          <label className="cursor-pointer bg-stone-100 hover:bg-stone-200 p-2 rounded-sm transition-colors border border-stone-200">
            <Upload className="w-4 h-4 text-stone-600" />
            <input type="file" className="hidden" accept="image/*" onChange={handleFileChange} />
          </label>
        </div>
        {value && (
          <div className="relative w-full h-20 bg-stone-100 rounded-sm overflow-hidden border border-stone-200">
  ...
  return (
    <div className="min-h-screen font-sans selection:bg-[#D4AF37]/30" style={{ backgroundColor: colors.stone, color: colors.walnut }}>
      ...
      <button 
        ...
        className="fixed bottom-6 right-6 z-[100] bg-[#44403C] text-white p-4 rounded-full shadow-2xl hover:scale-110 transition-transform flex items-center space-x-2"
      >
        <Layout className="w-6 h-6" />
        <span className="font-bold uppercase tracking-widest text-xs">{showEditor ? 'Close Editor' : 'Edit Page'}</span>
      </button>
      ...
      <AnimatePresence>
        {showEditor && (
          <motion.div 
            ...
            animate={{ x: 0 }}
            exit={{ x: '100%' }}
            className="fixed top-0 right-0 w-full md:w-[450px] h-screen bg-white z-[90] shadow-2xl overflow-y-auto p-8 border-l border-stone-200"
          >
            ...
            <div className="mb-10">
              ...
              {content.atAGlance.items.map((item, idx) => (
                <div key={idx} className="mb-4 p-4 bg-stone-50 rounded-sm space-y-2">
                  ...
                  <input 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm"
                    value={item.label}
                    onChange={(e) => updateContent('atAGlance', 'items', e.target.value, idx, 'label')}
                    placeholder="Label"
                  />
                  <input 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm"
                    value={item.sub}
                    onChange={(e) => updateContent('atAGlance', 'items', e.target.value, idx, 'sub')}
              ...
              {content.vibe.cards.map((card, idx) => (
                <div key={idx} className="mb-6 p-4 bg-stone-50 rounded-sm space-y-3">
                  ...
                  <input 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm"
                    value={card.title}
                    onChange={(e) => updateContent('vibe', 'cards', e.target.value, idx, 'title')}
                    placeholder="Card Title"
                  />
                  <textarea 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm h-20"
                    value={card.desc}
                    onChange={(e) => updateContent('vibe', 'cards', e.target.value, idx, 'desc')}
                    placeholder="Description"
                  />
                  <ImageUpload 
                    label="Card Image"
                    value={card.img}
                    onChange={(val) => updateContent('vibe', 'cards', val, idx, 'img')}
                  />
                </div>
              ))}
            </div>
              ...
              {content.units.types.map((type, idx) => (
                <div key={idx} className="mb-6 p-4 bg-stone-50 rounded-sm space-y-3">
                  ...
                  <input 
                    ...
                    value={type.name}
                    onChange={(e) => updateContent('units', 'types', e.target.value, idx, 'name')}
                  />
                  <input 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm"
                    value={type.size}
                    onChange={(e) => updateContent('units', 'types', e.target.value, idx, 'size')}
                  />
                  <input 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm"
                    value={type.rooms}
                    onChange={(e) => updateContent('units', 'types', e.target.value, idx, 'rooms')}
                  />
                  <ImageUpload 
                    label="Unit Layout Image"
                    value={type.image}
                    onChange={(val) => updateContent('units', 'types', val, idx, 'image')}
                  />
                </div>
              ))}
            </div>
              ...
              {content.facilities.images.map((img, idx) => (
                <div key={idx} className="mb-4 space-y-2 p-3 bg-stone-50 rounded-sm">
                  <ImageUpload 
                    ...
                    value={img.url}
                    onChange={(val) => updateContent('facilities', 'images', val, idx, 'url')}
                  />
                  <input 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm"
                    value={img.alt}
                    onChange={(e) => updateContent('facilities', 'images', e.target.value, idx, 'alt')}
              ...
              {content.location.items.map((item, idx) => (
                <div key={idx} className="mb-4 p-4 bg-stone-50 rounded-sm space-y-2">
                  <input 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm font-bold"
                    value={item.title}
                    onChange={(e) => updateContent('location', 'items', e.target.value, idx, 'title')}
                  />
                  <textarea 
                    className="w-full p-2 border border-stone-200 rounded-sm text-sm h-16"
                    value={item.desc}
                    onChange={(e) => updateContent('location', 'items', e.target.value, idx, 'desc')}
                  />
                </div>
              ))}
              ...
              {content.location.amenities.map((cat, catIdx) => (
                <div key={catIdx} className="mb-6 p-4 border border-stone-100 rounded-sm space-y-3">
                  <input 
                    ...
                    value={cat.category}
                    onChange={(e) => updateContent('location', 'amenities', e.target.value, catIdx, 'category')}
                    placeholder="Category Name"
                  />
                  {cat.list.map((item, itemIdx) => (
                    <div key={itemIdx} className="flex gap-2">
                      <input 
                        className="flex-1 p-2 border border-stone-200 rounded-sm text-xs"
                        value={item.name}
                        onChange={(e) => {
                          const newList = [...cat.list];
                          newList[itemIdx].name = e.target.value;
                          updateContent('location', 'amenities', newList, catIdx, 'list');
                        }}
                        placeholder="Amenity Name"
                      />
                      <input 
                        className="w-24 p-2 border border-stone-200 rounded-sm text-xs"
                        value={item.distance}
                        onChange={(e) => {
                          const newList = [...cat.list];
                          newList[itemIdx].distance = e.target.value;
                          updateContent('location', 'amenities', newList, catIdx, 'list');
                        }}
            ...
            {/* Footer Editor */}
            <div className="mb-10">
              <h3 className="text-xs uppercase tracking-widest font-bold text-[#D4AF37] mb-4">Footer</h3>
              <div className="space-y-4">
                <textarea 
                  className="w-full p-3 border border-stone-200 rounded-sm text-sm h-24"
            ...
            <p className="text-[10px] text-stone-400 uppercase tracking-widest text-center mt-12">Changes are applied in real-time</p>
          </motion.div>
        )}
      </AnimatePresence>
